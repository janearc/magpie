...
def serve(host: str = "127.0.0.1", port: int = 8092, inbox: Path | None = None,
    ...
    provider = FilesystemProvider(
        inbox,
        suffixes=_AUDIO_EXTS,
        notify_dir=pipeline.DATA_ROOT / "notifications",
        # the delivery log lives under magpie's ~/var, NOT in the inbox (a dropper there must
        # not be able to pre-seed it to suppress intake).
        state_path=pipeline.DATA_ROOT / "delivered.json",
    )
    threading.Thread(
